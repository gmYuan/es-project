 ...
 * 基本是光棍，或者说只是一个值；而对象类型是若干个属性的集合
 * 一切引用类型都是对象，函数和数组也是对象
 *
 * function为什么这么特殊？和其它对象的本质区别是什么？
 * function本质上来说是可以生产别的对象的，它是一个对象的工作，所有的对象（包括函数本身）都是函数生产出来的
...
//把批量创建出来的对象(构造函数实例)共用的属性, 放在构造函数的原型上
...
//我们可以对象的属性分成二种。
// 以Person.有些属性是每个特有的，都不一样。另外一种是所有的Person都一样的
//为了保证灵活，为了节约内存和性能。
